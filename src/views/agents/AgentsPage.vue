...
const createRules: FormRules = {
  id: [
    { required: true, message: t('pages.agents.form.idRequired'), trigger: 'blur' },
    { pattern: /^[a-zA-Z0-9_-]+$/, message: t('pages.agents.form.idPattern'), trigger: 'blur' },
  ],
}
...
const tableColumns = computed<DataTableColumns<AgentInfo>>(() => [
    ...
    render(row) {
      ...
      return h(NSpace, { align: 'center', size: 12 }, {
        default: () => [
          ...
          h('div', {}, [
            h(NText, { strong: true, style: 'display: block;' }, { default: () => identity?.name || row.name || row.id }),
            h(NText, { depth: 3, style: 'font-size: 12px;' }, { default: () => row.id }),
          ]),
        ],
      })
    },
  },
  {
    ...
    render(row) {
      ...
      return h(NTag, {
        ...
        default: () => isActive ? t('pages.agents.status.active') : t('pages.agents.status.idle'),
      })
    },
  },
  {
  ...
  },
  {
    ...
    render(row) {
      ...
      return theme ? h(NTag, { size: 'small', bordered: false, round: true }, { default: () => theme }) : '-'
    },
  },
  {
    ...
    render(row) {
      ...
      return model ? h(NText, {}, { default: () => model }) : h(NText, { depth: 3 }, { default: () => '-' })
    },
  },
  {
    ...
    render(row) {
      ...
      return h('div', { style: 'display: flex; flex-wrap: wrap; margin: -2px;' }, tags)
    },
  },
  {
...
<template>
  <div class="agents-page">
    <NCard class="agents-hero" :bordered="false">
      ...
      <NGrid cols="1 s:2 m:4" responsive="screen" :x-gap="10" :y-gap="10">
        <NGridItem>
          <NCard embedded :bordered="false" class="agents-metric-card">
            <NSpace justify="space-between" align="center">
              <NText depth="3">{{ t('pages.agents.metrics.totalAgents') }}</NText>
              <NIcon :component="PeopleOutline" />
            </NSpace>
            <div class="agents-metric-value">{{ agentStore.agents.length }}</div>
          </NCard>
        </NGridItem>
        <NGridItem>
          <NCard embedded :bordered="false" class="agents-metric-card">
            <NSpace justify="space-between" align="center">
              <NText depth="3">{{ t('pages.agents.metrics.totalSessions') }}</NText>
        ...
        <NGridItem>
          <NCard embedded :bordered="false" class="agents-metric-card">
            <NSpace justify="space-between" align="center">
              <NText depth="3">{{ t('pages.agents.metrics.totalTokens') }}</NText>
              <NText depth="3">{{ t('pages.agents.units.tokens') }}</NText>
            </NSpace>
            <div class="agents-metric-value">{{ formatTokenTotalK(totalTokens) }}</div>
          </NCard>
        </NGridItem>
        <NGridItem>
          <NCard embedded :bordered="false" class="agents-metric-card">
            <NSpace justify="space-between" align="center">
              <NText depth="3">{{ t('pages.agents.metrics.configuredModels') }}</NText>
    ...
    <NCard :title="t('pages.agents.listTitle')" class="agents-card">
      ...
      <NAlert v-if="!agentStore.supportsAgents" type="warning" :bordered="false" style="margin-bottom: 16px;">
        {{ t('pages.agents.notSupported') }}
      </NAlert>

      <NAlert v-if="agentStore.error" type="error" :bordered="false" style="margin-bottom: 16px;">
        {{ agentStore.error }}
      </NAlert>
      ...
      <NDataTable
        ...
        striped
      />
      <NEmpty v-else :description="t('pages.agents.empty')" style="padding: 48px 0;" />

      <NText v-if="agentStore.lastUpdatedAt" depth="3" style="font-size: 12px; display: block; margin-top: 16px;">
        {{ t('pages.agents.lastUpdated', { time: formatRelativeTime(agentStore.lastUpdatedAt) }) }}
      </NText>
    </NCard>
    ...
    <NModal
      ...
      :mask-closable="false"
    >
      <NForm ref="formRef" :model="createForm" :rules="createRules" label-placement="left" label-width="100">
    ...
    <NModal
      ...
      :mask-closable="false"
    >
      <NForm ref="identityFormRef" :model="identityForm" label-placement="left" label-width="80">
    ...
    <NModal
      ...
      :mask-closable="false"
    >
      <NForm ref="modelFormRef" :model="modelForm" label-placement="left" label-width="80">
